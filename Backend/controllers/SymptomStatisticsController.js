...
exports.get_symptom_logs_by_healthcare_worker = async (req, res) => {
    ...
    let patients = await CaseInvestigation.find({
        assigned_to: req.query.assigned_to
    });

    let patient_ids = [];
    patients.forEach(patient => patient_ids.push(patient.patient_id));

    let selected_users = await Patient.find({
        _id: {$in: patient_ids}
    });
    
    let selected_ids = [];
    selected_users.forEach(user => selected_ids.push(user._id));
